...
class ListEditorDialog : DialogFragment()
{
    ...
    companion object
    {
        fun newInstance(valueSetId : ValueSetId,
                        ...
                        sheetContext : SheetContext) : ListEditorDialog
        {
    ...
    override fun onCreateDialog(savedInstanceState : Bundle?) : Dialog
    {
        ...
        if (sheetContext != null)
        {
            ...
            Log.d("***LIST EDITOR", "set dialog view")
                              ...
                              savedInstanceState : Bundle?) : View?
    {
        ...
        if (sheetContext != null && valueSetId != null && updateTarget != null)
        {
            val sheetUIContext  = SheetUIContext(sheetContext, context)
            ...
            when (valueSet) {
                is Val -> {
                    val viewBuilder = ListEditorViewBuilder(valueSet.value,
                                                            ...
                                                            sheetUIContext,
                                                            this)
                    Log.d("***LIST EDITOR", "building dialog view")
...
object ListEditor
{
    ...
    fun valueView(sheetUIContext : SheetUIContext) : LinearLayout
    {
        ...
        layout.addView(this.checkboxView(sheetUIContext))

        val mainLayout = this.valueMainViewLayout(sheetUIContext)

        // > Header
        mainLayout.addView(this.valueHeaderView(sheetUIContext))

        // Summary
        mainLayout.addView(this.valueSummaryView(sheetUIContext))

        // Reference Link
        mainLayout.addView(this.referenceView(sheetUIContext))
    ...
    private fun checkboxView(sheetUIContext : SheetUIContext) : LinearLayout
    {
        // (1) Declarations
        // -------------------------------------------------------------------------------------
    ...
    private fun valueMainViewLayout(sheetUIContext : SheetUIContext) : LinearLayout
    {
        val layout = LinearLayoutBuilder()
                     ...
                     : RecyclerView.Adapter<ListEditorValueViewHolder>()
{
    ...
    {
        val itemView = ListEditor.valueView(sheetUIContext)
        return ListEditorValueViewHolder(itemView, sheetUIContext)
    }
                     ...
                     : RecyclerView.Adapter<RecyclerView.ViewHolder>()
{
    ...
    override fun onCreateViewHolder(parent : ViewGroup, viewType : Int) : RecyclerView.ViewHolder =
        when (viewType)
        {
            HEADER ->
            {
                val headerView = ListEditor.valueSetNameView(sheetUIContext)
                HeaderViewHolder(headerView)
            }
            // VALUE
            else ->
            {
                val valueView = ListEditor.valueView(sheetUIContext)
 ...
 */
class ListEditorValueViewHolder(itemView : View, val sheetUIContext: SheetUIContext)
...
//    var refView     : LinearLayout? = null
    ...
    val unselectedNameColor = SheetManager.color(sheetUIContext.sheetId, unselectedNameColorTheme)

    val unselectedSummaryColorTheme = ColorTheme(setOf(
            ThemeColorId(ThemeId.Dark, ColorId.Theme("light_grey_7")),
            ThemeColorId(ThemeId.Light, ColorId.Theme("dark_grey_15"))))
    val unselectedSummaryColor = SheetManager.color(sheetUIContext.sheetId, unselectedSummaryColorTheme)

    val selectedNameColorTheme = ColorTheme(setOf(
            ThemeColorId(ThemeId.Dark, ColorId.Theme("dark_grey_8")),
            ThemeColorId(ThemeId.Light, ColorId.Theme("light_blue"))))
    val selectedNameColor = SheetManager.color(sheetUIContext.sheetId, selectedNameColorTheme)
...
//    val unselectedBgColor = SheetManager.color(sheetUIContext.sheetId, selectedBgColorTheme)
    ...
    val checkSelectedColorTheme = ColorTheme(setOf(
            ThemeColorId(ThemeId.Dark, ColorId.Theme("dark_grey_8")),
            ThemeColorId(ThemeId.Light, ColorId.Theme("light_blue"))))
    val checkSelectedColor = SheetManager.color(sheetUIContext.sheetId, checkSelectedColorTheme)

    val checkUnselectedColorTheme = ColorTheme(setOf(
            ThemeColorId(ThemeId.Dark, ColorId.Theme("dark_grey_8")),
            ThemeColorId(ThemeId.Light, ColorId.Theme("light_grey_3"))))
    val checkUnselectedColor = SheetManager.color(sheetUIContext.sheetId, checkUnselectedColorTheme)
